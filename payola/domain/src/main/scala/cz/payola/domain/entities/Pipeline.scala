 ...
 */
class Pipeline(protected var _name: String, protected var _owner: Option[User], protected var _analysis: Analysis)
    extends Entity
    with NamedEntity
    with OptionallyOwnedEntity
    with cz.payola.common.entities.Pipeline
{
     ...
     */
    def addDataSource(ds: DataSourceType) {
        addRelatedEntity(ds, dataSources, storeDataSource)
    }

    /**
     * Removes the specified data source from the pipeline.
     * @param ds The data source to be removed.
     * @return The removed data source.
     */
    def removeDataSource(ds: DataSourceType): Option[DataSourceType] = {
        removeRelatedEntity(ds, dataSources, discardDataSource)
    }
