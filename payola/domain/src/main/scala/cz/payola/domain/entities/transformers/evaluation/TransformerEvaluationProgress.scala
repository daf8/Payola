 ...
 */
case class TransformerEvaluationProgress(evaluatedInstances: immutable.Seq[TransformerPluginInstance],
    runningInstances: Map[TransformerPluginInstance, Double], pendingInstances: immutable.Seq[TransformerPluginInstance],
    errors: Map[TransformerPluginInstance, Throwable])
{
    /**
     ...
     */
    private[evaluation] def withChangedProgress(instance: TransformerPluginInstance, value: Double): TransformerEvaluationProgress = {
        if (!evaluatedInstances.contains(instance)) {
            ...
            TransformerEvaluationProgress(evaluated, running, pending, errors)
        } else {
            this
        }
    }
     ...
     * @return The new analysis evaluation progress.
     */
    private[evaluation] def withError(instance: TransformerPluginInstance, throwable: Throwable): TransformerEvaluationProgress = {
        TransformerEvaluationProgress(evaluatedInstances, runningInstances, pendingInstances,
