...
class PluginDialog(plugins: Seq[Plugin]) extends Modal("Choose a type of plugin", Nil, None)
{
    ...
    val createTransformerPluginClicked = new SimpleUnitEvent[PluginDialog]
    ...
    private val createTransformerPluginAnchor = new Anchor(List(new Text("create")))
    ...
    createTransformerPluginAnchor.mouseClicked += { e =>
        createTransformerPluginClicked.triggerDirectly(this)
        false
    }
    ...
    //private val createTransformerPlugin = new Paragraph(List(new Text("... or "),createTransformerPluginAnchor,new Text(" a new plugin from an existing transformer")))
