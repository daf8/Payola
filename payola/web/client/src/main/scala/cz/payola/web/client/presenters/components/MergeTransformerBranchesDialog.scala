...
class MergeTransformerBranchesDialog(instances: ArrayBuffer[TransformerPluginInstanceView], inputsCount: Int)
    extends Modal("Choose how you want to merge the branches")
{
    val outputToInstance = new HashMap[Int, TransformerPluginInstanceView]
    ...
    def bindInstance(element: html.Element, instance: TransformerPluginInstanceView) {}

    @javascript(""" jQuery(element).data("inputIndex", index); """)
    def bindIndex(element: html.Element, index: Int) { }

    def setInstance(index: Int, instance: TransformerPluginInstanceView) = {
